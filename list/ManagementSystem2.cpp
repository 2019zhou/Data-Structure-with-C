...
int main()
{
	...
	double RW[20] = { 0 };
	int temp = 0;
	for (Node* p = head->next; p != NULL && temp <= 20; p = p->next) {
	...
	sort(RW, RW + number);
	int l = 0;
	for (Node* p = head->next; p != NULL && l <= 10; p = p->next) {
...
void add() {
	...
	//将员工信息写入文件
	fstream f1;
	f1.open("data.txt", ios::out | ios::app);//app表明是向文档末尾写信息
	if (f1) {
...
void deleting() {
	...
	if (cnt >= number) {
		...
		f3.open("data.txt");
		if (f3) {
			for (Node* f = head->next; f != NULL; f = f->next) {  
